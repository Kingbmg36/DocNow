...
> **Repo is pushed to GitHub** (`Kingbmg36/DocNow`, `main` branch). Along the way, found and fixed: (1) `public/index.html` carried the Emergent "Made with Emergent" badge, an `assets.emergent.sh` script tag, and an inline PostHog snippet hardcoded to **Emergent's own project key** — meaning a naive deploy would have sent this app's traffic/session recordings to Emergent's analytics account, not the user's; all three removed. (2) `Login.jsx`'s demo-account hint still showed `doctor@medinest.africa`/`admin@medinest.africa` — stale from before the rebrand, didn't match the actual seeded `@docnow.ng` accounts; fixed.
> **Render backend build fix:** the first real Render deploy failed at `pip install` — `requirements.txt` pinned `pytest==9.0.3`, which conflicts with `pytest-asyncio==0.24.0`'s `pytest<9` requirement. Neither package is ever imported by the running app (only `tests/`), so they moved to a new `backend/requirements-dev.txt` (installs on top of `requirements.txt`, pytest pinned `>=8.2,<9`) — the production Docker image no longer installs test tooling at all. `ci.yml` updated to install `requirements-dev.txt` for the test job.
> **Deploy platform: switched Railway → Render.** The Railway attempt (`DEPLOY.md`, still present for reference) hit real platform-side build flakiness — deploys stuck in `QUEUED` for 15+ min with zero build output despite Railway saying the "upstream GitHub issues" causing it were resolved; direct CLI (`railway up`) deploys failing at `scheduling build` with zero output, reproduced 3x; and mid-session the service was silently renamed/re-ID'd (`DocNow` → `natural-nurturing`), which reset its Root Directory and Builder settings. That last one is a real instance of dashboard-setting drift, which is exactly what Render's `render.yaml` Blueprint avoids — Docker build scope lives in version-controlled YAML, not a clickable field. See `DEPLOY-RENDER.md` + `render.yaml` (two services: `docnow-backend` Docker, `docnow-frontend` native static site).
